...
@section('content')
    <div class="mb-3">
        <h1>Edytuj odczyt medium #{{ $mediaUsage->id }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('media_usages.update', $mediaUsage) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select id="apartment_id" name="apartment_id" class="form-select" required>
                ...
                @foreach($apartments as $id => $number)
                    <option value="{{ $id }}"
                        {{ old('apartment_id', $mediaUsage->apartment_id) == $id ? 'selected' : '' }}>
                        Mieszkanie #{{ $number }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select id="media_type_id" name="media_type_id" class="form-select" required>
                ...
                @foreach($media_types as $id => $name)
                    <option value="{{ $id }}"
                        {{ old('media_type_id', $mediaUsage->media_type_id) == $id ? 'selected' : '' }}>
                        {{ $name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select id="rental_agreement_id" name="rental_agreement_id" class="form-select">
                ...
                @foreach($rental_agreements as $id => $label)
                    <option value="{{ $id }}"
                        {{ old('rental_agreement_id', $mediaUsage->rental_agreement_id) == $id ? 'selected' : '' }}>
                        Umowa #{{ $label }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="date"
                   ...
                   name="reading_date"
                   class="form-control"
                   value="{{ old('reading_date', $mediaUsage->reading_date->format('Y-m-d')) }}"
                   required>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number"
                   ...
                   name="value"
                   class="form-control"
                   value="{{ old('value', $mediaUsage->value) }}"
                   required>
        </div>
        ...
        <div class="mb-3 form-check">
            <input type="checkbox"
                ...
                {{ old('archived', $mediaUsage->archived) ? 'checked' : '' }}>
