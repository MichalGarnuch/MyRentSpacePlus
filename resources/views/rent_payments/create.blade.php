@extends('layouts.app')

@section('content')
    <div class="mb-3">
        <h1>Dodaj płatność czynszową</h1>
    </div>

    @if ($errors->any())
        <div class="alert alert-danger">
            <ul class="mb-0">
                @foreach ($errors->all() as $err)
                    <li>{{ $err }}</li>
                @endforeach
            </ul>
        </div>
    @endif

    <form method="POST" action="{{ route('rent_payments.store') }}">
        @csrf

        <div class="mb-3">
            <label for="rental_agreement_id" class="form-label">Umowa najmu:</label>
            <select id="rental_agreement_id" name="rental_agreement_id" class="form-select" required>
                <option value="">— Wybierz umowę —</option>
                @foreach($rental_agreements as $id => $label)
                    <option value="{{ $id }}" {{ old('rental_agreement_id') == $id ? 'selected' : '' }}>
                        Umowa #{{ $label }}
                    </option>
                @endforeach
            </select>
        </div>

        <div class="mb-3">
            <label for="payment_date" class="form-label">Data płatności:</label>
            <input type="date"
                   id="payment_date"
                   name="payment_date"
                   class="form-control"
                   value="{{ old('payment_date', now()->format('Y-m-d')) }}"
                   required>
        </div>

        <div class="mb-3">
            <label for="amount" class="form-label">Kwota (PLN):</label>
            <input type="number"
                   step="0.01"
                   id="amount"
                   name="amount"
                   class="form-control"
                   value="{{ old('amount') }}"
                   required>
        </div>

        <div class="mb-3">
            <label for="type" class="form-label">Typ płatności:</label>
            <select id="type" name="type" class="form-select" required>
                <option value="rent"      {{ old('type') == 'rent' ? 'selected' : '' }}>Czynsz</option>
                <option value="deposit"   {{ old('type') == 'deposit' ? 'selected' : '' }}>Kaucja</option>
                <option value="utilities" {{ old('type') == 'utilities' ? 'selected' : '' }}>Media</option>
            </select>
        </div>

        <div class="mb-3">
            <label for="status" class="form-label">Status:</label>
            <select id="status" name="status" class="form-select" required>
                <option value="pending"   {{ old('status') == 'pending' ? 'selected' : '' }}>Oczekuje</option>
                <option value="paid"      {{ old('status') == 'paid' ? 'selected' : '' }}>Opłacone</option>
                <option value="overdue"   {{ old('status') == 'overdue' ? 'selected' : '' }}>Zaległe</option>
            </select>
        </div>

        <button type="submit" class="btn btn-success">Zapisz</button>
        <a href="{{ route('rent_payments.index') }}" class="btn btn-secondary">Anuluj</a>
    </form>
@endsection
