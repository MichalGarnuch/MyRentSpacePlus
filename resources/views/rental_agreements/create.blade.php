...
@section('content')
    ...
    <form method="POST" action="{{ route('rental_agreements.store') }}">
        @csrf
        ...
        <div class="mb-3">
            ...
            <select id="status" name="status" class="form-select" required>
                ...
                <option value="terminated"{{ old('status') == 'terminated' ? 'selected' : '' }}>Zakończona</option>
                <option value="pending"   {{ old('status') == 'pending' ? 'selected' : '' }}>Oczekiwanie</option>
            </select>
        </div>
